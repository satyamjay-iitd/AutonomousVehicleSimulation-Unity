...
public class TrafficSystemVehicle : MonoBehaviour 
{
	...
	public  float                       m_vehicleCheckDist     = 5.0f;                      
	public  float                       m_vehicleCheckRadius   = 2.0f;
	[Range(0.0f, 100.0f)]
...
//			print ("TrafficLight: " + TrafficLight);
		...
		if(!StopMoving && m_nextNode && !CrashDetected)// && !m_pathingStarted)
		{
			...
			if(VehicleInSights)
			{
				...
				{
					// do nothing as these two cars are facing each other for unknown reasons so none of them should slow down with this ray detection.
				}
				else
				{
...
//
	...
	public TrafficSystemVehicle CanSeeVehicleViaRay( bool a_checkVehicleIsMoving = false )
	{
		...
		{
			TrafficSystemVehicle vehicle = hitInfo.transform.GetComponent<TrafficSystemVehicle>();
			TrafficSystemVehiclePlayer vehiclePlayer = hitInfo.transform.GetComponent<TrafficSystemVehiclePlayer>();
